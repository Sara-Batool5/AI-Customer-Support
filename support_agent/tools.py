...
class KnowledgeSearchTool(BaseTool):
    ...
    def _run(self, query: str) -> str:
        try:
            ...
            print(f"[search_company_knowledge] query={query!r} ids={ids[0].tolist()} scores={scores[0].tolist()}")
            ...
            # Surface the real error instead of silently looking like "no results found" —
            # this shows up in Streamlit Cloud's app logs (Manage app -> Logs) so it can be debugged
            # without needing to run anything locally.
            import traceback
            print("[search_company_knowledge] ERROR:")
            traceback.print_exc()
            return f"The knowledge base search tool hit an error: {e}"
